...
export default function Mentor() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-2 sm:p-4 font-sans transition-colors duration-200">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-[1000px] mx-auto overflow-hidden transition-colors duration-200">
        <div className="flex flex-col md:flex-row border-t dark:border-gray-700">
          {/* Left Panel (Chat List) */}
          <div className="md:w-1/3 w-full border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 p-4">
            <div className="flex justify-between mb-4">
              <button
                className={`font-medium ${activeTab === 'CHAT' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-300'}`}
                onClick={() => setActiveTab('CHAT')}
              >
                CHAT
              </button>
              <button
                className={`font-medium ${activeTab === 'CALLS' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-300'}`}
                onClick={() => setActiveTab('CALLS')}
              >
                CALLS
              </button>
            ...
            <div className="relative mb-4">
              ...
              <input
                type="text"
                placeholder="Search faculty"
                className="pl-10 pr-3 py-2 w-full border border-gray-300 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-100"
              />
            </div>
            {facultyList.map((faculty) => (
              <div
                ...
                className={`p-2 rounded-lg cursor-pointer transition ${
                  selectedChat === faculty.name
                    ? 'bg-blue-100 dark:bg-blue-900'
                    : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                }`}
              >
                <p className="font-medium dark:text-gray-100">{faculty.name}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{faculty.time}</p>
              </div>
            ))}
          </div>

          {/* Right Panel (Chat Box) */}
          <div className="md:w-2/3 w-full p-4 flex flex-col justify-between bg-gray-50 dark:bg-gray-900 transition-colors duration-200 min-h-[350px]">
            <div>
              <div className="flex justify-between items-center border-b dark:border-gray-700 pb-2 mb-4">
                <h3 className="font-semibold dark:text-gray-100">{selectedChat}</h3>
                <FaPhone className="text-gray-500 dark:text-gray-300" />
              </div>
              <div className="space-y-3 max-h-[40vh] min-h-[200px] overflow-y-auto pr-2">
                {messages.map((msg, index) => (
                  <div
                    key={index}
                    className={`flex ${msg.sender === 'Devansh' ? 'justify-end' : 'justify-start'}`}
                  >
                    <div
                      className={`p-2 rounded-lg text-sm w-fit max-w-xs break-words ${
                        msg.sender === 'Devansh'
                          ? 'bg-blue-200 dark:bg-blue-900 text-blue-900 dark:text-blue-200'
                          : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-100'
                      }`}
                    >
                      <p>{msg.text}</p>
                      <p className="text-[10px] text-right text-gray-500 dark:text-gray-400 mt-1">{msg.time}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center mt-4 border-t dark:border-gray-700 pt-2">
              <input
                type="text"
                placeholder="Type a message"
                className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-l-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-100"
              />
              <button className="bg-blue-500 hover:bg-blue-600 dark:bg-blue-700 dark:hover:bg-blue-800 text-white px-4 py-2 rounded-r-lg transition">
                <FaPaperPlane />
              </button>
            </div>
          </div>
