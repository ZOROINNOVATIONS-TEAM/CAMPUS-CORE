  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="font-semibold text-gray-800">Upcoming Dues</div>
        ...
        {dues.map((d, i) => (
          <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
            ...
            <div className="flex flex-col items-end">
              <div className="text-base font-bold text-gray-800">{d.amount}</div>
              <button className={`mt-2 px-4 py-1 text-xs font-semibold rounded ${d.action === "Pay Now" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}>{d.action}</button>
            </div>
          </div>
        ))}
      </div>
