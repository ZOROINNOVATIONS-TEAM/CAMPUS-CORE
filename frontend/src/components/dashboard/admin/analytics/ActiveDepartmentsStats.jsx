...
export default function ActiveDepartmentsStats() {
  return (
    <Card className="w-full h-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
      <CardHeader>
        ...
        <div className="flex flex-col gap-4">
          {departments.map((d, i) => (
            <div key={i} className="flex items-center justify-between">
              ...
              <span className="text-base font-bold text-gray-900 dark:text-white">{d.count}</span>
            </div>
          ))}
        </div>
      </CardContent>
